...
function mod!(g::sym_element, T::translation_group)
    mod(x) = modf(x)[1]  # Fractional part
    nvec = mod.(inv(T.basis) * g.gVec)  # Convert to fractional coordinates
    g.gVec .= g.gVec - T.basis * nvec   # Map back to reduced coordinates
end

# Overloading equality for sym_element
import Base.==
function ==(g1::sym_element, g2::sym_element)::Bool
    return g1.gVec ≈ g2.gVec && g1.gMat ≈ g2.gMat
end
...
"""
function find_order(g::sym_element)::Int
    e = neutral_elem(g)
    order = 0
    gn = e
...
function ==(bond1::bond, bond2::bond)::Bool
    if bond1.r1 ≈ bond2.r1 && bond1.r2 ≈ bond2.r2
        return true
    end
    if bond1.r1 ≈ bond2.r2 && bond1.r2 ≈ bond2.r1
        return true
    end
    return false
end
...
"""
function mod!(b::bond, T::translation_group)
    mod(x) = modf(x)[1]  # Get fractional part
    nvec = mod.(inv(T.basis) * b.r1)  # Convert r1 to fractional coordinates
...
function sym_reduced_lattice(lattice::Lattice, center_sites, G::sym_group, T::translation_group)
    ...
    for (it, b) in enumerate(bond_vec_red)
        ...
        for g in G.elements
            bnew = g ∘ b
            mod!(bnew, T)  # Normalize to unit cell

            mat_ind = findfirst(x -> x == bnew, bmat)

            # Record symmetry equivalence
            push!(reduction_dict, mat_ind => it)
            ...
            # Remove symmetry equivalent bonds from reduced vector
            filter!(x -> x ≠ bnew, bond_vec_red)
        end
    end
