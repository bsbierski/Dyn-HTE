...
the graph must be connected. Used for representing vacuum graphs.
"""
...
unique_Graph represents an equivalence class of graphs. 
...
# Description
It represents all multigraphs that have the same underlying simple graph structure.
Therefore, the embedding factor of unique_Graph is the same as all multigraphs in that equivalence class.
"""
...
Wraps all information about a lattice prepared for high-temperature series expansion calculations.
...
- `basis_positions::Vector{<:Int}`: Indices of basis/reference sites in the central unit cell
...
Can be generated by [`getLattice`](@ref).
"""
