#include Lattice support from SpinMC
include("Lattice.jl")
...
function getLattice(L::Int,geometry::String; PBC::Bool = true)
    """ creates lattice and corresponding graphs, L is the linear size, 
    PBC sets the use of boundary conditions.
        - chain
        - square
        - triang
        - honeycomb
...
latticeNoPBC = LatticeNoPBC(uc, l);
...
else error("geometry: " * geometry * " not yet implemented") 
    
end

if PBC #create lattice with or withou periodic Boundary conditions
lattice = Lattice(uc, l);
else
...
function getLattice_Ball(L::Int,geometry::String)
    """ 
    Gives the lattice where all sites are at most L away from the center sites
    """
...
lattice,LatGraph = getLattice(10,"honeycomb"; PBC = false);
display(graphplot(LatGraph,names=1:nv(LatGraph),markersize=0.2,fontsize=7,nodeshape=:rect,curves=false)) =#