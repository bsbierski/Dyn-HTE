...
#########################################################################################
###### Matsubara correlator at m=0,1,2 in real-space, comparison to QMC
#########################################################################################

###### load QMC data at m=0,1,2 (have x=[1,2,...,8]) 
...
for x_QMC_pos in eachindex(x_vec_QMC) 
    ...
    fid = h5open("examples/Chain/Chain_S1half_Matsubara_QMC_Worm/job_BSb_beta"*string(x_QMC)*".out.h5", "r")
...
if true
    ...
    plt_m0=Plots.plot(x_vec,0*x_vec,color=:black,lw=0.8,xlims=(0,x_vec[end]),ylims=(-0.12,0.26), xformatter=Returns(""),ylabel=L"TG_{ii^\prime}(i\nu_{m=0})",  label="",legend=:topright)
    plt_m1=Plots.plot(x_vec,0*x_vec,color=:black,lw=0.8,xlims=(0,x_vec[end]),ylims=(-0.035,0.04),xformatter=Returns(""),ylabel=L"TG_{ii^\prime}({i\nu_{m=1}})",label="",legend=:bottomleft)
    plt_m2=Plots.plot(x_vec,0*x_vec,color=:black,lw=0.8,xlims=(0,x_vec[end]),ylims=(-0.01,0.02),xlabel=L"x=J/T",ylabel=L"TG_{ii^\prime}(i\nu_{m=2})",   label="",legend=:topleft)
    ...
    for di in di_vec, (m_pos,m) in enumerate(m_vec)
        ...
        Plots.plot!(plt_vec[m_pos],x_vec_bare,p_x.(x_vec_bare),label=label,color=:grey,linewidth=0.5)
        ...
        for  (pade_pos,pade) in enumerate(pade_vec)
            ...
            Plots.plot!(plt_vec[m_pos],x_vec,get_pade(p_u,pade[1],pade[2]).(u_vec),color=:purple,alpha=0.6,linestyle=linestyle_vec[pade_pos],label=label)
        end
    end

    ### plot QMC data
    for di in di_vec, (m_pos,m) in enumerate(m_vec)
        Plots.scatter!(plt_vec[m_pos],x_vec_QMC,TGiip_m_QMC[:,1+di,1+m],color=color_vec[di+1],label="",markersize=6)
    ...
    ### put panels together
    xPlots,yPlots=1,3
    plt_final = Plots.plot(plt_vec[1],plt_vec[2],plt_vec[3],  layout=(yPlots,xPlots), size=(aps_width*xPlots,0.4*aps_width*yPlots), dpi=600)
    display(plt_final)
    #savefig(plt_final,"examples/Chain/Chain_S1half_Matsubara_r-space.png")
end
...
if true
    ...
    ### prepare panels for m=0,1,2
    plt_m0=Plots.plot(xlims=(0,x_vec[end]),ylims=(0,0.75),xformatter=:none,ylabel=L"TG_{k}(i\nu_{m=0})",  label="",legend=:topleft)
    plt_m1=Plots.plot(xlims=(0,x_vec[end]),ylims=(0,0.10),xformatter=:none,ylabel=L"TG_{k}({i\nu_{m=1}})",label="",legend=:topleft)
    plt_m2=Plots.plot(xlims=(0,x_vec[end]),ylims=(0,0.04),xlabel=L"x=J/T",ylabel=L"TG_{k}(i\nu_{m=2})",   label="",legend=:topleft)
    plt_vec = [plt_m0,plt_m1,plt_m2]
    ...
    for (koπ_pos,koπ) in enumerate(koπ_vec), (m_pos,m) in enumerate(m_vec)
        ...
        if !(m!=0 && koπ==0) Plots.scatter!(plt_vec[m_pos],x_vec_QMC,TGk_m_QMC,color=color_vec[7+koπ_pos],label="",markersize=6,marker=:square) end
        if m==0 Plots.annotate!(plt_vec[m_pos],x_vec_QMC[2]+0.1,TGk_m_QMC[2]+0.1-(koπ_pos-1)*0.18,Plots.text("k/π="*string(koπ),7,color_vec[7+koπ_pos])) end
    end

    ### put panels together
    xPlots,yPlots=1,3
    plt_final = Plots.plot(plt_vec...,  layout=(yPlots,xPlots), size=(aps_width*xPlots,0.4*aps_width*yPlots), dpi=600)
    display(plt_final)
    #savefig(plt_final,"examples/Chain/Chain_S1half_Matsubara_k-space.png")
end