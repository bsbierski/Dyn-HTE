...
#display(graphplot(hte_lattice.graph,names=1:nv(hte_lattice.graph),markersize=0.2,fontsize=5,curves=false))
...
#calculate the δs of the continued fraction expansion 
...
# extrapolate the deltas to infinity via
deltas_at_x_ext = []
for x_idx = 1:length(x_vec)
    δ_vec_ext = extrapolate_δvec(deltas_at_x[x_idx],3,3,1000,true)
    push!(deltas_at_x_ext,δ_vec_ext)
end

#add the extrapolated deltas to the plot 
for (i,x) in enumerate(x_vec)
    Plots.scatter!(plt_δ,4:10,deltas_at_x_ext[i][5:11],markers=:cross,label = "",title="δs extrapolated",color = thermalCol4_vec[i],markersize=4,alpha=0.5)
end
display(plt_δ)
...
DSF = [JSwithTerminator(deltas_at_x[x_idx],x_vec[x_idx],w,get_extrapolation_params(deltas_at_x[x_idx],3,3,true)) for w in w_vec]
...
JSkw_mat = get_JSkw_mat("u_pade",x,k_vec,w_vec,c_iipDyn_mat,hte_lattice,r_min=3,r_max=3,f=0.48)
